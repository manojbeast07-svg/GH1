"""Basic CUDA Laplacian filter demonstration + CPU-vs-GPU comparison.

Usage:
    python scripts/test_laplacian_cuda.py --image path/to/xray.jpg --kernel-size 3 --scale 1.0 --delta 0.0
...
(Laplacian's conversion is deterministic round+clamp, so max_abs_diff is
expected to be exactly 0 -- verified empirically, not a tolerance),
prints cold/warm GPU timings plus CPU timing, and saves
...
from cuda.benchmark import benchmark_laplacian_single_image  # noqa: E402
from cuda.laplacian import laplacian_cuda  # noqa: E402
...
def main() -> int:
    ...
    parser.add_argument("--save-dir", type=str, default="outputs/laplacian_cuda_demo")
    ...
    cpu_output = apply_laplacian(image, config)
    cuda_output = laplacian_cuda(image, kernel_size=args.kernel_size, scale=args.scale, delta=args.delta)

    diff = np.abs(cpu_output.astype(np.int16) - cuda_output.astype(np.int16))
    max_diff = int(diff.max())
    num_differing = int(np.count_nonzero(diff))
    correct = max_diff == 0  # exact-equality goal (Section 4E spec item 14, verified achievable)

    bench = benchmark_laplacian_single_image(
        image, kernel_size=args.kernel_size, scale=args.scale, delta=args.delta,
    ...
    cv2.imwrite(str(save_dir / "cpu_laplacian.png"), cpu_output)
    cv2.imwrite(str(save_dir / "cuda_laplacian.png"), cuda_output)
    diff_vis = np.clip(diff * 32, 0, 255).astype(np.uint8)
