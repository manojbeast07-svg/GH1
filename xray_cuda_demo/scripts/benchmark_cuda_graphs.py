"""Section 20D: benchmarks the EXPERIMENTAL CudaGraphPipeline (CUDA
Graph capture/replay for the five BASIC filter kernels) against two
baselines, on the real dataset:

  A. TRUE production pipeline -- cuda.pipeline.run_basic_cuda_pipeline()
     (all-Basic defaults), completely unmodified.
  B. This experiment's own use_graph=False path -- identical host code
     to the graph path minus capture/replay, isolating exactly the
     graph-vs-no-graph variable.
  C. This experiment's use_graph=True path, both scopes (KernelsOnly
     and FullPipeline).

Deliberately never touches xray_cuda.PersistentCudaPipeline or pinned
memory (spec items 16-18: isolate the graph-specific hypothesis from
Section 20B/20C's already-decided persistence/pinned-memory findings).

Two measurement modes:

  --batch-sweep 1,8,32,64,122   Steady-state repeated-call timing per
                                  batch size (same shape/config repeated
                                  --warmup-runs discarded, --runs measured).

  --realistic-workload           The SAME varied-parameter/varied-batch/
                                  varied-image sequence, and the SAME
                                  fixed methodology (context warmup,
                                  discarded full-sequence warmup passes,
                                  alternating execution order), that
                                  Section 20C used to find persistent
                                  buffers' 28% regression -- reused here
                                  unchanged so this section's finding is
                                  directly comparable, not measured with
                                  a differently-biased method.

Usage:
    python scripts/benchmark_cuda_graphs.py --dataset ../data --seed 42 --batch-sweep 1,8,32,64,122
    python scripts/benchmark_cuda_graphs.py --dataset ../data --seed 42 --realistic-workload
"""
...
RESULTS_DIR = PROJECT_ROOT / "benchmark_results" / "research_optimization" / "cuda_graphs"
...
def _graph_call(pipeline, batch, config, gc, lc, sm, use_graph, full_pipeline_scope):
    return pipeline.run(
        batch, True, gc, True, config.median_kernel_size, True, sm,
        True, lc, config.laplacian_scale, config.laplacian_delta,
...
def benchmark_one_batch_size(images_pool, config, gc, lc, sm, batch_size, warmup_runs, measurement_runs):
    imgs = images_pool[:batch_size]
    batch = np.stack(imgs, axis=0)
    expected, _ = run_basic_cuda_pipeline(imgs, config)

    prod_times = []
    for i in range(warmup_runs + measurement_runs):
        out, t = run_basic_cuda_pipeline(imgs, config)
        if i >= warmup_runs:
    ...
    try:
        ...
        variant_correct = {}
        for i in range(warmup_runs + measurement_runs):
            r = _graph_call(pipeline, batch, config, gc, lc, sm, use_graph=False, full_pipeline_scope=True)
            if i >= warmup_runs:
                variant_times["no_graph"].append(r["total_ms"])
            variant_correct["no_graph"] = bool(np.array_equal(r["output"], expected))
        pipeline.clear_cache()
        for i in range(warmup_runs + measurement_runs):
            r = _graph_call(pipeline, batch, config, gc, lc, sm, use_graph=True, full_pipeline_scope=False)
            if i >= warmup_runs:
        ...
        first_instantiate_ms = None
        pipeline.clear_cache()
        for i in range(warmup_runs + measurement_runs):
            r = _graph_call(pipeline, batch, config, gc, lc, sm, use_graph=True, full_pipeline_scope=True)
            if i == 0:
...
def realistic_workload_benchmark(images_pool, config, gc, lc, sm, passes: int):
    """Reuses Section 20C's fixed measurement methodology (context warmup,
    discarded full-sequence warmup passes for every variant, alternating
    execution order across measured passes) against the same style of
    varied-parameter/varied-batch/varied-image sequence that revealed
    persistent buffers' 28% regression -- so this section's number is
    directly comparable, not an artifact of a differently-biased method."""
    _warm_context()

    def sequence(pipeline, use_graph):
        # select batch(32) -> process -> change threshold -> process(same shape)
        # -> change batch size to 8 -> process -> select a new batch(64) -> process
        b32 = np.stack(images_pool[:32], axis=0)
        _graph_call(pipeline, b32, config, gc, lc, sm, use_graph, True)
        import dataclasses
        config2 = dataclasses.replace(config, threshold_value=200)
        _graph_call(pipeline, b32, config2, gc, lc, sm, use_graph, True)
        b8 = np.stack(images_pool[32:40], axis=0)
        _graph_call(pipeline, b8, config, gc, lc, sm, use_graph, True)
        b64 = np.stack(images_pool[40:104] if len(images_pool) >= 104 else images_pool[:64], axis=0)
        _graph_call(pipeline, b64, config, gc, lc, sm, use_graph, True)

    def stateless_sequence():
        b32 = images_pool[:32]
        run_basic_cuda_pipeline(b32, config)
        import dataclasses
        config2 = dataclasses.replace(config, threshold_value=200)
        run_basic_cuda_pipeline(b32, config2)
        b8 = images_pool[32:40]
        run_basic_cuda_pipeline(b8, config)
        b64 = images_pool[40:104] if len(images_pool) >= 104 else images_pool[:64]
        run_basic_cuda_pipeline(b64, config)
    ...
    graph_pipeline = xray_cuda.CudaGraphPipeline()
    try:
        # 2 discarded full-sequence warmup passes for both variants
        for _ in range(2):
            stateless_sequence()
...
def main() -> int:
    ...
    parser.add_argument("--batch-sweep", type=str, default=None, help="Comma-separated batch sizes, e.g. 1,8,32,64,122")
    ...
    print("=" * 70)
    print("SECTION 20D -- CUDA GRAPH PIPELINE OPTIMIZATION EXPERIMENT")
    print("=" * 70)
    ...
    if batch_sizes:
        ...
        for batch_size in batch_sizes:
            if batch_size > len(images_pool):
                ...
                continue
            print(f"\n--- batch_size={batch_size} ---")
            result = benchmark_one_batch_size(images_pool, config, gc, lc, sm, batch_size, args.warmup_runs, args.runs)
    ...
    if args.realistic_workload:
        if len(images_pool) < 104:
            print(f"\nWarning: realistic workload needs 104 same-resolution images, only {len(images_pool)} available; "
                  "batch(64) step will reuse a smaller slice.")
        print(f"\n--- REALISTIC WORKLOAD (decisive test, {args.realistic_passes} passes, alternating order) ---")
        realistic = realistic_workload_benchmark(images_pool, config, gc, lc, sm, args.realistic_passes)
        print(f"  stateless (production) total: median={realistic['stateless_total_ms']['median']:.4f}ms")
        print(f"  graph (CudaGraphPipeline) total: median={realistic['graph_total_ms']['median']:.4f}ms")
        print(f"  gain: {realistic['gain_graph_vs_stateless']:.3f}x")
        manifest["realistic_workload"] = realistic

    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    experiment_id = "cuda_graphs_" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%S")
    manifest["experiment_id"] = experiment_id
    out_path = RESULTS_DIR / f"{experiment_id}.json"
    with open(out_path, "w", encoding="utf-8") as fh:
        json.dump(manifest, fh, indent=2, default=str)
    with open(RESULTS_DIR / "latest.json", "w", encoding="utf-8") as fh:
        json.dump(manifest, fh, indent=2, default=str)
