"""Basic vs. Enhanced CUDA Laplacian A/B benchmark (Section 9).

Usage:
    python scripts/benchmark_laplacian_optimization.py --dataset ../data --batch-size 125 --kernel-size 3 --scale 1.0 --delta 0 --runs 20

Measures Basic Laplacian against the four Enhanced variants (shared-
memory tiling, constant-memory coefficients, compile-time
matrix-size specialization, hand-written explicit arithmetic) using the
SAME batched, single-native-call architecture Sections 5-8 established
(grid.z = batch dimension) -- never single-image timing as the primary
decision metric. Also runs a block/tile configuration sweep for the
winning variant, and reports the full-pipeline effect (Enhanced
Laplacian + Enhanced Gaussian/Median/Sobel vs. all-Basic).

Correctness is checked at every step -- Laplacian's coefficients are all
small exact integers (2, 4, -8, -24, ...) with no fractional weights, so
this script uses EXACT equality (max_abs_diff == 0) against both CPU and
Basic CUDA, not a tolerance, for every kernel_size/scale/delta/variant.
"""
...
def main() -> int:
    ...
    parser.add_argument("--kernel-size", type=int, default=3, choices=[1, 3, 5])
    ...
    print("=" * 70)
    print("BASIC vs ENHANCED CUDA LAPLACIAN BENCHMARK")
    print("=" * 70)
    print(f"\nResolution: {shape[1]}x{shape[0]}   Batch: {n} images   "
          f"kernel_size={args.kernel_size} scale={args.scale} delta={args.delta}")
    print(f"Warmup: {args.warmup_runs}   Measurement runs: {args.runs}")

    config = FilterConfig(laplacian_kernel_size=args.kernel_size, laplacian_scale=args.scale, laplacian_delta=args.delta)
    coeffs = laplacian_kernel_2d(args.kernel_size)
    actual_k = coeffs.shape[0]

    # Unused placeholders for the disabled Gaussian/Median/Sobel stages
    # -- required positional args to run_basic_cuda_pipeline_gpu, but
    # never read because those stages are disabled below.
    ...
    def bench_basic():
        ...
        for i in range(args.warmup_runs + args.runs):
            r = xray_cuda.run_basic_cuda_pipeline_gpu(
                ...
                True, coeffs, args.scale, args.delta, False, 128, 255)
    ...
    def bench_enhanced(variant_int, block=(16, 16)):
        ...
        out = None
        for i in range(args.warmup_runs + args.runs):
            r = xray_cuda.laplacian_enhanced_batch_gpu(batch, coeffs, args.scale, args.delta, variant_int, block[0], block[1])
            if i >= args.warmup_runs:
                vals.append(r["kernel_ms"])
            out = r["output"]
        return vals, out

    print(f"\nActual coefficient matrix shape: {coeffs.shape} (kernel_size={args.kernel_size} is OpenCV's "
          f"aperture parameter, not necessarily the matrix dimension -- see cuda/laplacian.py)")
    ...
    cpu_expected = np.stack([apply_laplacian(img, config) for img in images])

    table_rows = [("Basic", basic_med, 1.0, "PASS (reference)")]
    for name in LAPLACIAN_VARIANTS:
        vint = laplacian_variant_to_int(name)
        try:
            times, out = bench_enhanced(vint)
        except Exception as exc:  # e.g. Explicit rejecting an unrecognized coefficient set
            print(f"{name:30s} SKIPPED ({exc})")
            table_rows.append((name, None, None, f"SKIPPED ({exc})"))
            continue
    ...
    ranked = [row for row in table_rows[1:] if row[1] is not None]
    winner_name = max(ranked, key=lambda row: row[2])[0] if ranked else "shared"
    print(f"\n--- Block/tile tuning ({winner_name} variant) ---\n")
    for bx, by in [(8, 8), (16, 16), (32, 8), (32, 16)]:
        times, out = bench_enhanced(laplacian_variant_to_int(winner_name), block=(bx, by))
        med = _median_ms(times)
        print(f"block={bx}x{by:<4d} median={med:.4f}ms")

    print("\n--- Full-pipeline effect (Enhanced Laplacian + Basic Gaussian/Median/Sobel/Threshold) ---\n")
    full_config = FilterConfig()  # all defaults, all 5 stages enabled
    for _ in range(args.warmup_runs):
        run_basic_cuda_pipeline(images, full_config, use_enhanced_laplacian=False)
        run_basic_cuda_pipeline(images, full_config, use_enhanced_laplacian=True, laplacian_variant=winner_name)

    basic_pipeline_totals, enhanced_pipeline_totals = [], []
    for _ in range(args.runs):
        _out_b, t_b = run_basic_cuda_pipeline(images, full_config, use_enhanced_laplacian=False)
        basic_pipeline_totals.append(t_b.total_ms)
        _out_e, t_e = run_basic_cuda_pipeline(images, full_config, use_enhanced_laplacian=True, laplacian_variant=winner_name)
